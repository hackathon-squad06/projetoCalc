function calcular() {
  ...
  const valorizacao = parseFloat(
    document.getElementById("inputValorizacao").value
  );
  ...
  const rentabilidade = parseFloat(
    document.getElementById("inputRentabilidade").value
  );
  ...
  const parcela =
    valorFinanciado *
    (((1 + iFin / 100) ** n * iFin) / 100 / ((1 + iFin / 100) ** n - 1));
  ...
  const m =
    c * (1 + iF / 100) ** n +
    valorInvestidoMensalF * (((1 + iF / 100) ** n - 1) / (iF / 100));
...
document
  .getElementById("inputValorImovel")
  .addEventListener("change", function () {
